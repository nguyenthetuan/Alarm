...
import { IGarageDetail } from 'types';
import { formatDistanceKm, formatMoney, getImage } from 'utils';

interface IProps extends IGarageDetail {
  onPress: () => void;
  isDetail?: boolean;
  price?: number;
  number_rentals?: number;
  basePrice?: number;
  loading: boolean;
  avatar?: string;
}

const VehicleCategoryItem: React.FC<IProps> = ({
  onPress,
  isDetail,
  name,
  distance,
            ...
            <>
              <AppSkeleton height={20}>
                <TextCus>
                  ...
                  <TextCus bold> {`${number_rentals || 0}`} lần</TextCus>
                </TextCus>
              </AppSkeleton>
              <ViewCus flex-row>
